...
export async function POST(req:NextRequest){
  try{
    ...
    if(profile.ttsProvider==='vexyl' && process.env.VEXYL_TTS_URL){
      ...
      const description=languageCode==='sa'
        ? 'Vasudha speaks Sanskrit clearly and naturally. Use a calm native Sanskrit teaching voice with precise vowel length and aspirated consonants.'
        : undefined;
      const submit=await fetch(`${base}/batch/synthesize`,{method:'POST',headers,body:JSON.stringify({text,lang:profile.locale,style:'default',...(description?{description}:{})})});
      if(!submit.ok)return jsonError(`VEXYL TTS failed (${submit.status}). Check VEXYL_TTS_URL and that the Sanskrit model is loaded.`,502);
      const job=await submit.json();
      if(!job?.job_id)return jsonError('VEXYL TTS did not return a job id',502);
      for(let i=0;i<30;i++){
        ...
        if(data.status==='failed')return jsonError('VEXYL TTS job failed while synthesizing Sanskrit',502);
      }
      return jsonError('VEXYL TTS timed out while generating Sanskrit audio',504);
    }

    // Sarvam Bulbul v3 for supported Indian languages. Sanskrit is intentionally not sent here.
    ...
    if(process.env.OPENAI_API_KEY){
      ...
      if(!response.ok){
        ...
        return jsonError(`OpenAI TTS failed (${response.status})${details?`: ${details.slice(0,240)}`:''}`,502);
      }
      const buffer=Buffer.from(await response.arrayBuffer());
      return new NextResponse(buffer,{headers:{'Content-Type':'audio/mpeg','Cache-Control':'public,max-age=3600'}});
    }

    return NextResponse.json({provider:'browser',language:profile.locale,reason:`No server TTS provider is configured for ${profile.locale}. For reliable ${languageCode==='sa'?'Sanskrit':''} audio, configure OPENAI_API_KEY or VEXYL_TTS_URL.`});
